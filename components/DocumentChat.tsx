...
const DocumentChat: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex flex-col bg-white rounded-lg shadow-sm border overflow-hidden">
      <div className="p-4 bg-[#0B3B2E] text-white flex items-center gap-2 shadow-sm z-10">
        <div className="bg-[#F4A024] p-1.5 rounded">
           <Bot className="w-5 h-5 text-[#0B3B2E]" />
        </div>
        <div>
           <h2 className="font-bold text-sm md:text-base">LINKS Transaction AI</h2>
           <p className="text-[10px] md:text-xs text-green-100 opacity-90">Acquisitions • Diligence • Integration</p>
        </div>
      </div>
        ...
        {history.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[90%] md:max-w-[80%] rounded-lg p-4 shadow-sm ${
              ...
              <div className="whitespace-pre-wrap text-sm leading-relaxed prose prose-sm max-w-none prose-p:my-1 prose-headings:text-[#0B3B2E] prose-ul:my-1 prose-li:my-0">
                {msg.content}
              </div>
                ...
                <div className="mt-4 pt-3 border-t border-gray-100">
                  ...
                  <div className="space-y-2">
                    {msg.evidence.map((doc, i) => (
                      <div key={i} className="flex gap-3 bg-gray-50 p-2 rounded border border-gray-200 hover:bg-gray-100 transition-colors">
                        <FileText className="w-8 h-8 text-[#0B3B2E] flex-shrink-0 opacity-80" />
                        <div className="overflow-hidden min-w-0">
                          <p className="text-xs font-bold text-[#0B3B2E] truncate" title={doc.filename}>{doc.filename}</p>
                          <div className="flex gap-2 text-[10px] text-gray-500 mt-0.5">
                             <span className="bg-gray-200 px-1.5 rounded text-gray-700">{doc.type || 'DOC'}</span>
          ...
          <div className="flex justify-start">
            <div className="bg-white p-4 rounded-lg rounded-bl-none shadow-sm border border-gray-200 flex items-center gap-3">
              <div className="flex space-x-1">
                <div className="w-2 h-2 bg-[#0B3B2E] rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                <div className="w-2 h-2 bg-[#00A86B] rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                <div className="w-2 h-2 bg-[#F4A024] rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
              </div>
      ...
      <div className="p-4 bg-white border-t">
        <div className="relative max-w-4xl mx-auto">
          <input 
            type="text" 
            className="w-full pl-4 pr-12 py-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#00A86B] focus:border-transparent outline-none text-sm transition-all"
          ...
          <button 
            ...
            disabled={loading}
            className="absolute right-2 top-2 p-1.5 bg-[#0B3B2E] text-white rounded hover:bg-[#00A86B] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Send className="w-4 h-4" />
          </button>
        </div>
        <p className="text-[10px] text-center text-gray-400 mt-2">
          LINKS Transaction AI v1.5 • Accessing 5 Live Datasets • Verify critical info in VDR.
        </p>
      </div>
    </div>
  );
