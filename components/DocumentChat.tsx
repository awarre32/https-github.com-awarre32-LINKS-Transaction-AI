...
import { Send, FileText, Search, Bot, Paperclip, Loader2 } from 'lucide-react';
...
const DocumentChat: React.FC = () => {
  const { documents, taskStatus, roadmap, checklist, monday, currentDeptView, currentDealFilter } = useData();
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        setHistory(prev => [...prev, {
          role: 'ai',
          content: `✅ Successfully uploaded **${file.name}**. I am now analyzing it...`
        }]);
      } catch (error) {
        console.error(error);
        setHistory(prev => [...prev, {
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 bg-[#F5F7FA]">
        {history.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[90%] md:max-w-[80%] rounded-lg p-4 shadow-sm ${msg.role === 'user'
      ...
      <div className="p-4 bg-white border-t border-gray-100">
        <div className="relative max-w-4xl mx-auto">
          ...
          <button
            ...
            className="absolute right-12 top-2 p-1.5 text-gray-400 hover:text-[#006747] transition-colors disabled:opacity-50"
